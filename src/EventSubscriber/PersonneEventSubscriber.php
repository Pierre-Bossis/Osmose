...
use Symfony\Component\EventDispatcher\EventSubscribeInterface;
...
class PersonneEventSubscriber implements EventSubscriberInterface
{
    ...
    public function onAddPersonneEvent(AddPersonneEvent $event)
    {
        ...
        $mailMessage = $personne->getFirstname().' '.$personne->getName().' a été ajouté avec succès.'/*$message*/;
        $this->logger->info('Envoi d email pour '. $personne->getFirstname().' '.$personne->getName());
        $this->$mailer->sendEmail(content: $mailMessage, subject:'mail send from EventSubscriber !');
    }
}